	...
	public partial class MostrarUsuario : Form
	{
		...
		void MostrarUsuarioLoad(object sender, EventArgs e)
		{
			DataTable memoria = new DataTable();
            	try
            	{
            	    NpgsqlConnection conexion = new NpgsqlConnection("Server=localhost;"+ "Database=biblioteca;" +"User ID=Kevin;");
            	    NpgsqlDataAdapter datosConsulta = new NpgsqlDataAdapter("select idusuario,nombre,librosprestados,contrasena from usuario", conexion);
